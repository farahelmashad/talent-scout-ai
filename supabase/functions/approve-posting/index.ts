...
// Lazy-loaded embedding pipeline (384 dimensions with gte-small)
let embeddingPipe: any = null;

async function getEmbeddingPipeline() {
  if (!embeddingPipe) {
    console.log('🔄 Initializing gte-small embedding pipeline (384 dimensions)...');
...
serve(async (req) => {
  ...
  try {
    ...
    // Get embedding using local pipeline (384 dimensions)
    console.log('🔄 Getting 384D embedding...');
...
async function getEmbedding(text: string): Promise<number[]> {
  console.log('🔄 Generating 384D embedding with gte-small...');
  ...
  try {
    ...
    // Convert the output to a standard number array
    ...
    console.log('✅ Local embedding successful! Dimensions: 384');
    return embedding;
